...
import { readContent, writeContent } from './cms/store'
...
function ContentHydrator({children}:{children:ReactNode}) { const [ready,setReady]=useState(false); useEffect(()=>{ fetch('/api/content/homepage').then(response=>response.ok?response.json():null).then(remote=>{ if(remote && typeof remote==='object') writeContent(remote); setReady(true) }).catch(()=>setReady(true)) },[]); return ready ? <>{children}</> : <div className="content-loading" aria-label="Loading website"/> }

export default function App() { if (window.location.pathname.startsWith('/admin')) return <CmsErrorBoundary>{localStorage.getItem('cms-auth-token') ? <Admin/> : <AdminLogin/>}</CmsErrorBoundary>; return <ContentHydrator><Navbar/><main><Hero/><Categories/><Products/><Features/><About/><DynamicContact/><Testimonials/></main><DynamicFooter/></ContentHydrator> }
